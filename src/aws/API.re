...
//   };
// [@bs.send]
// external _graphqlQuery:
//   (t, Types.graphqlOperation) => Js.Promise.t(Js.t('a)) =
//   "graphql";
// let query =
//   graphqlOperation => {
//     _graphqlQuery(api, graphqlOperation);
//   };
...
//   message;
// };
// let extractCommentFrom = event => {
//   let event = event->Obj.magic;
//   /* get the message value on event and post to ui */
//   let message = event##value##data##comment##message;
//   message;
// };
// /* setting up like this returns the message on which we can call `setMessage()` */
// let subscribeToComment = graphqlOperation =>
//   _subscribe(api, graphqlOperation)
//   |> Wonka.fromObservable
//   |> Wonka.map((. event) => extractCommentFrom(event));
